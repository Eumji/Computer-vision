...
def main():
    ...
    for image_idx in range(len(input)):
        ...
        kernel_size_row = 3
        kernel_size_col = 3
        kernel = cv.getStructuringElement(cv.MORPH_RECT, (kernel_size_row, kernel_size_col))
        result = cv.medianBlur(result, 13)
        result = cv.erode(result, kernel, iterations=2)

        result = cv.medianBlur(result, 9)
        result = cv.dilate(result, kernel, iterations=3)
        ...
        ##### renew background
        # alpha = 0.001
        # frame_prev_gray = (1 - alpha) * frame_current_gray + alpha * frame_prev_gray

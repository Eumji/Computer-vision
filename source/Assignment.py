...
def main():
    ...
    for image_idx in range(len(input)):
        ...
        # 목적과 무관한 영역(풀, 나무)를 제거함
        for i in range(diff_abs.shape[0] - 1):
            for j in range(diff_abs.shape[1] - 1):
                if 210 > 1.3 * i + j:
                    diff_abs[i][j] = 0

        diff_abss = (diff_abs + diff_temp) / 2
        diff_temp = diff_abs

        ##### make mask by applying threshold
        frame_diff = np.where(diff_abss > threshold, 1.0, 0.0)
        ...
        # Use median filter * 2
        # 메디안 블러를 두번, Erosion과 Dilation 적용
        result = cv.medianBlur(result, 5)
        # result = cv.medianBlur(result, 9)
        # result = cv.morphologyEx(result, cv.MORPH_OPEN, kernel=np.ones((5, 5), np.uint8))
